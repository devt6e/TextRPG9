...
public:
    ...
    int GetMaxHp() const { return MaxHp; }          // 최대 체력 조회

    int GetMp() const { return Mp; }                // 현재 마나 조회
    int GetMaxMp() const { return MaxMp; }          // 최대 마나 조회
    ...
    int GetMaxExp() const { return MaxExp; }        // 레벨업 필요 경험치 조회
    ...
    // Setter
    void SetHp(int Value);                          // 현재 체력 수정
    void SetMaxHp(int Value) { MaxHp = Value; }     // 최대 체력 수정

    void SetMp(int Value);                          // 현재 마나 수정
    ...
    void SetJob(std::string Value) { Job = Value; } // 직업 수정
