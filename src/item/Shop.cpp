...
void ShopManager::SellItem(Player* player, InventoryManager& inventoryManager) {
    ...
    if (targetItem != nullptr) {
        std::cout << "몇 개를 파시겠습니까? (최대 " << targetItem->ItemCount << "개): ";
        int amount;
        std::cin >> amount;

        if (amount <= 0) {
            std::cout << "[오류] 1개 이상 판매해야 합니다.\n";
            return;
        }
        ...
        if (selectedBag->RemoveItem(sellChoice, amount)) {
            ...
            std::cout << "[System] " << sellPrice << "ZEM을 획득했습니다!\n";
        }
    }
}
...
void ShopManager::EnterShop(Player* player, InventoryManager& inventoryManager) {
    while (true) {
        ...
        if (choice == 1) {
            ...
            std::cout << "[오류] 잘못된 입력입니다. 다시 선택해주세요.\n";
        }
    }
}
