...
void UI::PrintStatus(Player* p)
{
    ...
    UI::Gotoxy(Offsets[0], ++Offsets[1]);
    std::cout << "보유 ZEM: " << p->GetGold(); // ysg: 상점에서 현재 소지금을 확인할 수 있도록 스탯 영역에 표시
    UI::Gotoxy(1, 24);
}
...
//분류: 몬스
    ...
    while (true) {
        ...
        if (expanding) {
            breathState++;
            if (breathState >= 2) expanding = false;
        }
        else {
            breathState--;
            if (breathState <= 0) expanding = true;
        }
...
namespace
{
    void RenderAsciiArt(UI& ui, const std::string& art)
    {
        ui.EraseArt();
        ...
        std::string line;
        int y = 2;
        constexpr int lastArtRow = 19;

        while (y <= lastArtRow && std::getline(lines, line))
        {
            if (y == 2 && line.empty())
            {
                continue;
            ...
            for (char character : line)
            {
                if (character == '\\')
                {
                    std::cout << "\xE2\x95\xB2"; // ysg: 한국어 콘솔에서 역슬래시가 원화로 보이지 않도록 대각선 문자 출력
                }
                else
                {
                    std::cout << character;
...
void UI::PrintMonsterArt(const std::string& monsterName)
{
    std::string art;

    if (monsterName == "일 안하는 ZEP 본사 직원")
    {
        art = R"ART(
        :::::
    #@*+++++++*@+
...
:%:=++++++.*:+++=:@
 ...
 :-.:#@%##%##%@*:.=:
  :##@%%%@@@%%%@##:)ART";
    ...
    else if (monsterName == "아침마다 모닝콜로 노래 부르는 수강생")
    {
        art = R"ART(
                @@@@@@@
              @         @
    ...
    else if (monsterName == "매니저님 남편 꿈나무")
    {
        art = R"ART(
            #######%%%@
          @######%@%%##%#
    ...
    else if (monsterName == "남자는_태어나서_세번_컴파일한다_정윤재")
    {
        art = R"ART(
      @%+        =#@
       %%@######@%*+
    ...
    RenderAsciiArt(*this, art);
}

void UI::PrintItemArt(const std::string& itemName)
{
    std::string art;
