...
void JYJ::SpeakBattleStart() const {
    std::cout << "남자는_태어나서_세번_컴파일한다_정윤재: \"첫 번째 컴파일은 태어날 때, 두 번째는 배포할 때, 마지막 세 번째는... 바로 지금이다!\"\n";
}

void JYJ::SpeakVictory() const {
    std::cout << "남자는_태어나서_세번_컴파일한다_정윤재: \"크하하! 에러 없는 클린 빌드의 맛을 보아라! 완벽해!\"\n";
}

void JYJ::SpeakDefeat() const {
    std::cout << "남자는_태어나서_세번_컴파일한다_정윤재: \"크윽... 빌드 에러 99개라니... 네 번 컴파일하게 생겼잖아...?!\"\n";
}

void JYJ::Attack() {
    std::cout << "남자는_태어나서_세번_컴파일한다_정윤재가 음침하게 남의 TIL을 염탐합니다!\n";
}